...
import docker
from schemas import Settings, FunctionIn, FunctionOut, FunctionOutList

app = FastAPI()
settings = Settings()


def deploy(id, payload, server):
    client = docker.DockerClient(base_url=f"tcp://{server}:2375")
    print(payload, id)
    image = client.images.build(
        path=".", buildargs={"payload": payload}, tag=id, quiet=False
    )
    return image
...
def create_function(function: FunctionIn):
    id = str(uuid.uuid4())
    try:
        deploy(id, function.payload, settings.docker_registry)
